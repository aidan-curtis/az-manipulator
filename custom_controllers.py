...
class PIDController:
    ...
    def angle_diff(self, d1, d2):
        angle_diff = d1-d2
        return min(angle_diff, angle_diff+math.pi*2, angle_diff-math.pi*2, key=lambda a: abs(a))

    def diff(self, d1, d2):
        return d1-d2

    def control(self, current, target, spherical=None):
        if(len(current.shape) == 0):
            err = target - current
        else:
            err = []
            for j in range(current.shape[0]):
                if(spherical is not None and j in spherical):
                    err.append(self.angle_diff(target[j], current[j]))
                else:
                    err.append(self.diff(target[j], current[j]))
...
class PositionController:
    ...
    def control(self, current, target, *args, **kwargs):
        """
        Args:
        ...
        """
        target_vel = self.velocity_pid.control(current, target, *args, **kwargs)
