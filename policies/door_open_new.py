...
class DoorNewPolicy(SingleAPolicy):
    ...
    def update_phase(self, phase, grasp, current_q,
        ...
        def door_opened_enough():
            ...
            return door_moved != None and (door_moved > self.estimated_door_width / 4) or door_moved > 0.15  ## door_moved > 0.15
    ...
    def get_prepush_xy(self, obs=None):
        if self.prepush_xy == None:
            ...
            if self.estimated_door_width > 0.3:
                ...
                self.prepush_xy = self.get_offset_xy(obs=obs, x_off=-0.05, y_off=0.2)  ## -0.2
            # if not self.handle_horizontal and self.prepush_xy[0] > self.body_x_min:
            #     self.prepush_xy[0] -= 0.05
    ...
    def check_door_moved(self, segmented_pc=None, obs=None):
        if self.door_moved == None:
            ...
            # print('self.door_moved', self.door_moved)
            if self.door_moved > 0.01 and self.maybe_open_left:
                for xyz in door_pc:
                    if xyz[0] == x_min:
    ...
    def get_center(self, pc):
        """ instead of using np.mean, find the (min+max)/2 """
        return (np.min(pc, axis=0) + np.max(pc, axis=0) ) / 2
    ...
    def check_away_from_handle(self, obs, epsilon=0.2):
        ...
        xyz_mean_door = self.get_center(handle_pc)
    ...
    def check_open_right(self, segmented_pc):
        ...
        handle_pc, door_pc, body_pc, robot_pc = segmented_pc
        door_center = self.get_center(door_pc)[1]
        body_center = self.get_center(body_pc)[1]
        handle_center = self.get_center(handle_pc)[1]
    ...
    def check_handle_horizontal(self, grasp_pc):
        ...
        if self.handle_horizontal == None or True:
            x_range, y_range, z_range = self.get_ranges(self.grasp_pc)
            if y_range > 2 * z_range:
                handle_horizontal = True
            elif z_range > 2 * y_range:
    ...
    def get_target_qpos(self, phase, grasp, env_name="OpenCabinetDrawer-v0", obs=None):
        ...
        if (phase == 0):  ## move to pregrasp
            ...
            # print('     current gripper', qpos[-2:])
            # print('     target gripper', fast_open_gripper)
    ...
    def generate_grasps(self, segmented_pc):
        ...
        mean_point = self.get_center(grasp_pc)
        # print('num of handle points', len(grasp_pc))
        # print('handle center', mean_point)
    ...
    def get_conf_data(self, grasp_pose, robots, name_link_dict, q0):
        ...
        if USE_DRAKE:
            ...
            grasp_q = p.calculateInverseKinematics(robots['center'], name_link_dict["right_panda_link8"], real_grasp_pose[0],
                                          real_grasp_pose[1])
    ...
    def get_pc(self, obs):
        ...
        if len(self.grasp_pc) < NUM_HANDLE_POINTS:
            self.grasp_pc = grasp_pc
    ...
    def act(self, obs):
        ...
        if self.grasps == None:
            ...
            body_pc = segmented_pc[2]
            self.body_x_min = np.min(np.asarray(body_pc), axis=0)[0]
            ...
            self.start_obs = obs
            self.obs_count = 0
            if len(self.grasp_pc) > 30:
                self.obs_count = 10
        elif self.obs_count < 10:
            self.get_pc(obs) ## add handle points
            self.obs_count += 1
            if len(self.grasp_pc) > NUM_HANDLE_POINTS:
                self.obs_count = NUM_HANDLE_POINTS
        elif self.obs_count == NUM_HANDLE_POINTS:
            self.grasps = self.get_grasps(obs, num=20)
            self.obs_count = 10
        ...
        center = copy.deepcopy(current_q[:3])
        center[:2] = [0,0]
